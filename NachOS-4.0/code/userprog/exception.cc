...
void ExceptionHandler(ExceptionType which)
{
	...
	case SyscallException:
		...
		case SC_Open:
		{
			...
			{
				DEBUG(dbgSys, "DEBUG: Can not open file: " << buf << "\n");
				kernel->machine->WriteRegister(2, -1);
			}
			else
			{
				DEBUG(dbgSys, "DEBUG: Open file successfully!");
				DEBUG(dbgSys, "DEBUG: FileName: " << buf << "\n  FileID: " << temp_index);
		...
		case SC_Close:
		{
			...
			if (result == -1)
			{
				DEBUG(dbgSys, "DEBUG: Fail to close!");
			}
			else
			{
				DEBUG(dbgSys, "DEBUG: Close file success!");
			}
