...
public:
	...
	~FileSystem()
	{
		for (int i = 0; i < FILE_DESCRIPTOR_TABLE_SIZE; i++)
		{
			if (fileDescriptorTable[i].openFile != NULL)
			{
				delete fileDescriptorTable[i].openFile;
			}

			if (fileDescriptorTable[i].filename != NULL)
			{
				delete fileDescriptorTable[i].filename;
			}
		}
	}
